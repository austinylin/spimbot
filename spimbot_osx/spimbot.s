# this implementation has a shared copy of "arctan" that is used by
# both the interrupt handler and the main program.

# so as not to overwrite any registers, when arctan is called from the
# interrupt handler, the interrupt handler saves all of the registers
# used by arctan, because those registers are caller saved.
...
main:   
    ...
    # flag for bonk & kick and global interrupt enable
    li      $t4, 0x3001     
...
search_end:
    ...
    bgt     $s0, 2, reset           # reset ball counter
    j       ball_search
...
interrupt_dispatch:             # Interrupt:
    ...
    and                 # add dispatch for other interrupt types here.
